...
% now construct the edge of the polygon
points = [x,y];
points(end+1,:) = points(1,:);
dp = points(2:end,:) - points(1:end-1,:);
ldp = sqrt(sum(dp.^2,2));
dp = [dp(:,1)./ldp,dp(:,2)./ldp];

n_points = size(x,1);
x_edge = cell(1,n_points);
y_edge = cell(1,n_points);
for i = 1 : n_points
    l_edge = floor(ldp(i));
    x_edge{i} = (0:l_edge)*dp(i,1)+x(i);
    y_edge{i} = (0:l_edge)*dp(i,2)+y(i);
end
